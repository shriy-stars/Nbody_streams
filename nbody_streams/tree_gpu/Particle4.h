...
struct Box
{
  ...
  __host__ __device__ Box(const Position<T> &c, T hs) : centre(c), hsize(hs) {}
};
...
{
  return (__float_as_int(packed_data.w) >> 4) & 0x0FFFFFFF;
}
...
{
  return ((unsigned long long)(packed_data.w) >> 4) & 0x0FFFFFFF;
}
